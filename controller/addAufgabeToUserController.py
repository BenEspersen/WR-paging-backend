...
def handler():
    ...
    if dbService.CheckPermissionViaToken(data.get("access_token"), data.get("token_owner")):
        dbService.addAufgabeToTechniker(data.get("username"), data.get("aufgabe"))
        dbService.drop()
        return {"status": "successful", "message": "successfully added aufgabe to techniker"}
    else:
        dbService.drop()
...
def checkRequest(data):
    if data is not None:
        return data.get("access_token") is not None and data.get("token_owner") is not None and \
               data.get("username") is not None and data.get("aufgabe") is not None
    else:
        return False
